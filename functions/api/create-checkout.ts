...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try { const body = await request.json() as { plan?: string }; if (body.plan !== 'monthly' && body.plan !== 'yearly') return json({ error: '不正なプランです' }, 400); plan = body.plan; } catch { return json({ error: 'JSON形式のリクエストが必要です' }, 400); }
  ...
  params.set('mode','subscription'); params.set('line_items[0][price]',price); params.set('line_items[0][quantity]','1'); params.set('success_url',`${origin}/?checkout=success&session_id={CHECKOUT_SESSION_ID}`); params.set('cancel_url',`${origin}/?checkout=cancelled`); params.set('client_reference_id',nonce); params.set('allow_promotion_codes','true'); params.set('locale','auto'); params.set('metadata[product]','cascade-circuit-premium'); params.set('metadata[plan]',plan);
